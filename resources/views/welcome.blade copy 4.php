...
<body>
    ...
    <div class="modal fade" id="aboutModal" tabindex="-1">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p>Gincaneiros é um jogo de adivinhação geográfica que desafia seu conhecimento de lugares ao redor do mundo.</p>
    ...
    <script>
        ...
        // Função para gerar um local aleatório com Street View disponível
        async function getRandomLocation() {
            const maxAttempts = 10;
            let attempts = 0;
            
            while (attempts < maxAttempts) {
                // Gera coordenadas aleatórias dentro de limites razoáveis
                const lat = Math.random() * 170 - 85; // -85 a 85
                const lng = Math.random() * 360 - 180; // -180 a 180
                
                // Verifica se há Street View disponível nessas coordenadas
                ...
                try {
                    ...
                    // Se encontrou Street View, retorna as coordenadas
                    return {
                        lat: panoramaData.location.latLng.lat(),
                        lng: panoramaData.location.latLng.lng(),
                        name: "Local aleatório"
                    };
                } catch (error) {
                    attempts++;
                    console.log(`Tentativa ${attempts}: Street View não encontrada, tentando novamente...`);
                }
            }
            
            // Se não encontrou após várias tentativas, retorna um local padrão
            console.log("Não foi possível encontrar um local com Street View após várias tentativas. Usando local padrão.");
            return {
                lat: -23.55052,
                lng: -46.633308,
            ...
            if (seedParam && locationsParam) {
                ...
                currentGameLocations = locationsCoords.map(coord => {
                    ...
                    return { 
                        lat: parseFloat(lat), 
                        lng: parseFloat(lng), 
                        name: `Local ${currentGameLocations.length + 1}` 
                    };
                });
            }
        ...
        async function newRound() {
            ...
            // Escolhe um local aleatório com Street View disponível
            correctLocation = await getRandomLocation();
            
            // Armazena o local para possível compartilhamento
            currentGameLocations.push({
                lat: correctLocation.lat,
                lng: correctLocation.lng,
                name: `Local ${roundsPlayed + 1}`
            });
        ...
        function endGame() {
            ...
            Swal.fire({
                ...
                willClose: () => {
                    // Zera os locais armazenados quando o modal fecha
                ...
                if (result.isDenied) {
                    ...
                    if (navigator.share) {
                        navigator.share({
                            title: 'Desafie-me no Gincaneiros!',
