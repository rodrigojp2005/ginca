...
<body class="bg-gray-100 pt-16">
    ...
    <script>
        ...
        let userState = null; // Estado do usuário será detectado
        let stateBounds = null; // Limites do estado do usuário
        let preloadedLocations = []; // Fila de pré-carregamento
                        ...
                        navigator.geolocation.getCurrentPosition(
                            async (position) => {
                                ...
                                try {
                                    ...
                                    stateBounds = new google.maps.LatLngBounds();
                                    const radius = 50 / 111.32; // Graus aproximados para 100km
        ...
        function getRandomBrazilLocation() {
          ...
          // Função para pré-carregar a próxima localização
        async function preloadNextLocation() {
            if (preloadedLocations.length < 3) { // Mantém até 3 locações pré-carregadas
                try {
                    const nextLocation = await getRandomLocation();
                    preloadedLocations.push(nextLocation);
                    console.log("Pré-carregada:", nextLocation.name);
                } catch (error) {
        ...
        // Função para obter uma nova localização aleatória
        ...
        async function initMap() {
            ...
            // Inicia o pré-carregamento de locações
            await preloadNextLocation();
            await preloadNextLocation();
            await preloadNextLocation();
        ...
        async function newRound() {
            ...
            // Usa a próxima localização pré-carregada, se disponível
            if (preloadedLocations.length > 0) {
                correctLocation = preloadedLocations.shift(); // Remove e usa a primeira locação da fila
            } else {
                // Se não houver locações pré-carregadas, gera uma nova localização na hora
                correctLocation = await getRandomLocation();
            }
            ...
            // Pré-carrega a próxima localização
            preloadNextLocation();
        }
