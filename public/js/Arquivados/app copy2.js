...
// }
    ...
    async function getRandomLocation() {
        ...
        while (attempts < maxAttempts) {
            try {
                // Modo cidade com geolocalização
                if (gameMode === 'city' && cityBounds) {
                    try {
                        const center = cityBounds.getCenter();
                        const places = await getNearbyPlaces(center.lat(), center.lng(), 20000);
                        
                        // 1. Filtro completo de lugares válidos
                        ...
                        if (validPlaces.length === 0) {
                            throw new Error("Nenhum local válido encontrado dentro dos limites da cidade");
                        }

                        // 2. Escolha aleatória entre lugares válidos
                        const randomPlace = validPlaces[Math.floor(Math.random() * validPlaces.length)];
                        const placeLocation = randomPlace.geometry.location;
                        
                        // 3. Verificação do panorama com checagem de bounds
                        const panoramaData = await getPanoramaData(
                            placeLocation.lat(), 
                            placeLocation.lng(),
                            100
                        ...
                        // Verificação extra de bounds para o panorama
                        if (!cityBounds.contains(panoramaData.location.latLng)) {
                            throw new Error("Panorama fora dos limites da cidade");
                        }
                        
                        // 4. Obter nome do lugar
                        const locationName = randomPlace.name || await getCityName(
                            placeLocation.lat(), 
                            placeLocation.lng()
                        );
                        
                        // 5. Adicionar ao cache de locais usados
                        usedLocations.push({
                            lat: placeLocation.lat(),
                            lng: placeLocation.lng()
                        });
                        ...
                        console.warn("Falha no modo cidade, tentando abordagem alternativa:", placeError);
                        
                        // Tentar novamente com busca direta no bounds
                        const lat = cityBounds.getSouthWest().lat() + Math.random() * 
                                (cityBounds.getNorthEast().lat() - cityBounds.getSouthWest().lat());
                        const lng = cityBounds.getSouthWest().lng() + Math.random() * 
                                (cityBounds.getNorthEast().lng() - cityBounds.getSouthWest().lng());
                        
                        const panoramaData = await getPanoramaData(lat, lng);
                        
                        // Verificação final de bounds
                        if (!cityBounds.contains(panoramaData.location.latLng)) {
                            throw new Error("Localização final fora dos bounds");
                        }
                ...
                // Restante do código para outros modos (mantido igual)
                let lat, lng;
        ...
        // Fallback extremo - para modo cidade, garantir que está dentro dos bounds
        console.warn("Usando fallback após", maxAttempts, "tentativas");
